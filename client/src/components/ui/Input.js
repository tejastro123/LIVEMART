import React, { useState } from 'react';
import './Input.css';

const Input = ({
  label,
  type = 'text',
  placeholder,
  value,
  onChange,
  name,
  error,
  success,
  icon,
  iconRight,
  showPasswordToggle = false,
  floatingLabel = false,
  className = '',
  ...props
}) => {
  const [isPasswordVisible, setIsPasswordVisible] = useState(false);
  const [isFocused, setIsFocused] = useState(false);

  const inputType = type === 'password' && showPasswordToggle && isPasswordVisible ? 'text' : type;
  const hasValue = value && value.length > 0;
  const isFloating = floatingLabel && (isFocused || hasValue);

  const togglePasswordVisibility = () => {
    setIsPasswordVisible(!isPasswordVisible);
  };

  return (
    <div className={`input-group ${className}`}>
      <div className="input-wrapper">
        {icon && <span className="input-icon input-icon-left">{icon}</span>}

        <input
          type={inputType}
          name={name}
          className={`input-field ${error ? 'input-error' : ''} ${success ? 'input-success' : ''} ${icon ? 'input-with-icon-left' : ''} ${iconRight || showPasswordToggle ? 'input-with-icon-right' : ''} ${floatingLabel ? 'input-floating' : ''}`}
          placeholder={floatingLabel ? '' : placeholder}
          value={value}
          onChange={onChange}
          onFocus={() => setIsFocused(true)}
          onBlur={() => setIsFocused(false)}
          {...props}
        />

        {floatingLabel && label && (
          <label className={`input-label-floating ${isFloating ? 'active' : ''}`}>
            {label}
          </label>
        )}

        {!floatingLabel && label && (
          <label className="input-label-static">{label}</label>
        )}

        {showPasswordToggle && type === 'password' && (
          <button
            type="button"
            className="input-icon input-icon-right password-toggle"
            onClick={togglePasswordVisibility}
            tabIndex={-1}
          >
            {isPasswordVisible ? (
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
              </svg>
            ) : (
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
            )}
          </button>
        )}

        {iconRight && !showPasswordToggle && (
          <span className="input-icon input-icon-right">{iconRight}</span>
        )}

        {success && !iconRight && !showPasswordToggle && (
          <span className="input-icon input-icon-right input-success-icon">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
            </svg>
          </span>
        )}

        {error && !iconRight && !showPasswordToggle && (
          <span className="input-icon input-icon-right input-error-icon">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </span>
        )}
      </div>

      {error && <span className="input-error-msg">{error}</span>}
      {success && <span className="input-success-msg">{success}</span>}
    </div>
  );
};

export default Input;
